...
function makeApiRequest(transport, opts, bodyObj, callback) {
  ...
  if (bodyObj) {
    try {
      var cache = [];
      writeData = JSON.stringify(bodyObj, function(key, value) {
  ...
  req.on('error', function(err) {
    util.error('[Rollbar] Could not make request to rollbar, ' + err);
    return callback(err);
  });
...
function parseApiResponse(respData, callback) {
  ...
  if (respData.err) {
    util.debug('Received error from rollbar: ' + respData.message);
