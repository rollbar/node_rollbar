...
function pushOntoRetryQueue() {
  retryQueue.push(arguments);

  if (!retryHandle) {
    retryHandle = setInterval(function() {
      while (retryQueue.length) {
        makeApiRequest.apply(null, retryQueue.splice(0, 1)[0]);
      }
...
function makeApiRequest(transport, opts, bodyObj, callback) {
  ...
  req.on('error', function (err) {
    ...
    if (SETTINGS.retryInterval && err.code == 'ENOTFOUND') {
      pushOntoRetryQueue.apply(null, originalArguments);
    } else {
