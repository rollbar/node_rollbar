...
exports.VERSION = '1';
exports.endpoint = 'https://submit.ratchet.io/api/' + exports.VERSION + '/';
...
exports.newAccount = function(username, email, callback) {
  return postApi('account/', {username: username, email: email}, callback);
};
...
function makeApiRequest(transport, opts, bodyObj, callback) {
  ...
  if (bodyObj) {
    ...
    opts.headers['Content-Length'] = writeData.length;
  }
...
function parseApiResponse(respData, callback) {
  try {
    ...
    util.error('could not parse api response, err: ' + e);
    return callback(e);
  }

  if (respData.err) {
    util.debug('Received error from ratchet: ' + respData.message);
    return callback(new Error('Api error: ' + (respData.message || 'Unknown error')));
  } else {
    util.debug('Successful api response');
