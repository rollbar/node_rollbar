...
var extractContextLines = function(frame, fileLines) {
  frame.pre_context = fileLines.slice(Math.max(0, frame.lineno - (linesOfContext + 1)), frame.lineno - 1);
  frame.context_line = fileLines[frame.lineno - 1];
  frame.post_context = fileLines.slice(frame.lineno, frame.lineno + linesOfContext);
};
