...
function buildBaseData(extra) {
  ...
  if (extra) {
    // Make a copy of `extra` to ensure that it isn't circular.
    var extraCopy;
    try {
      extraCopy = JSON.parse(JSON.stringify(extra));
    } catch (e) {
      // extra contains a circular reference - have to drop it.
      extraCopy = {custom: {internalError: "Circular reference inside passed payload data"}};
    }
    
    if (extraCopy) {
      data = rollbarutil.merge(data, extraCopy);
    }
  }
