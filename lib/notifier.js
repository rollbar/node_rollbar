...
exports.shutdown = function(callback) {
  ...
  waitForParsingItems(function(err) {
    console.error('executing wait callback');
    postItems(callback);
  });
};
  ...
  try {
    ...
    return parser.parseException(err, function(e, errData) {
      if (e) {
        ...
        delete parsingItems[parsingItems.indexOf(err)];
...
function waitForParsingItems(callback) {
  async.until(function () { return parsingItems.length == 0; },
              function () { console.log('waiting: ' + parsingItems.length); },
              callback);
}
