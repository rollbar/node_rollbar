...
Notifier.prototype = {
  ...
  parseEndpoint: function parseEndpoint(endpoint) {
    try {
      var parsed = url.parse(dsn), response = {
  ...
  addError: function(exc, params, callback) {
    ...
    payload['body'] = this.formatException(error);
  ...
  createAsyncHandler: function() {
    ...
    var _handler = function() {
      ...
      try {
        ...
        node_util.error('could not post item to ratchet.io');
      }
    };
    return _handler;
  },
  postItems: function(endpointOptions, items) {
    ...
    if (item) {
      ...
      node_util.debug('Posted error to Ratchet.io: ' + item.payload);
    }
  },
  ...
  parseException: function(exc) {
    var ret = {};
    var stack = this.parseStack(err.stack);
    ret['name'] = err.name;
    ret['message'] = err.message || '<no message>';
    ret['frames'] = stack;
  },
  parseFrame: function(frame) {
    return {
      filename: (frame.filename || 'unknown file').replace(process.cwd() + '/', ''),
      function: framefunction || 'unknown function'
    };
  },
        ...
        if (frame.filename[0] === '/' || frame.filename[0] === '.') {
          ...
          if (frame.filename in cache) {
            ...
            try {
              ...
              node_util.error('Could not open ' + frame.filename);
            }
          }
        } else {
          frames[index] = frame;
