...
exports.init = function(api, options) {
  ...
  if (SETTINGS.handler == 'setInterval') {
    intervalHandler = setInterval(processItems, SETTINGS.handlerInterval * 1000);
  }
};
...
function waitForParsingItems(callback) {
  async.until(function () { return parsingItems.length == 0; },
              function (cont) {
                console.log('waiting: ' + parsingItems.length);
                setTimeout(cont, 0);
              },
              callback);
}
...
function processItems(callback) {
  ...
  try {
    async.whilst(
      ...
      function (callback) {
        var numToRemove = Math.min(pendingItems.length, SETTINGS.batchSize);
        var batchItems = pendingItems.splice(0, numToRemove);

        async.map(batchItems, parseItem, function (err, results) {
          apiClient.postItems(results, function(err, resp) {
            if (!err) {
              resp.ids.forEach(
                function(val, index) {
                  responses.ids.push(val);
                }
              );
            }
            callback(err);
...
function parseItem(item, callback) {
  var baseData = buildBaseData(item.payload);
  ...
  async.series([
      ...
      function addErrorData(callback) {
        if (item.error) {
          parseErrorData(baseData, item.error, callback);
        } else {
          callback(null);
        }
      },
      ...
      function addRequestData(callback) {
        if (item.request) {
          SETTINGS.addRequestData(baseData, item.request);
        }
        callback(null);
      }
    ],
...
function parseErrorData(baseData, err, callback) {
  parser.parseException(err, function(e, errData) {
    ...
    baseData.server = buildServerData();
    callback(null);
  });
}
